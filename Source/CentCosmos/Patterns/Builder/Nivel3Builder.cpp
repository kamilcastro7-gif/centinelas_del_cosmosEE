...
void ANivel3Builder::AgregarAmbientacion(UWorld* World)
{
    ...
    // Nivel 3 — originales + Asteroides
    SpawnNaves(Gen, 8);
    SpawnSatelites(Gen, 8);
    SpawnRestos(Gen, 8);
    SpawnAsteroides(Gen, 6);

    if (GEngine)
        GEngine->AddOnScreenDebugMessage(-1, 3.f, FColor::White,
            TEXT("[Nivel3] Ambientación generada: 30 obstáculos."));
}
...
void ANivel3Builder::AgregarEnemigos(UWorld* World)
{
    ...
    // Ola 1: 14 Vástagos + 3 Vigías + 2 Heraldos + 2 BoyaElectrostatica
    SpawnVastagos(Factory, 14);
    SpawnVigias(Factory, 3);
    SpawnHeraldos(Factory, 2);
    SpawnBoyaElectrostatica(Factory, 2);

    if (GEngine)
        GEngine->AddOnScreenDebugMessage(-1, 3.f, FColor::Orange,
            FString::Printf(TEXT("[Nivel3] Ola 1 generada: %d enemigos."), EnemigosOla1.Num()));
}
...
void ANivel3Builder::SpawnVastagos(AEnemyFactory* Factory, int32 Cantidad)
{
...
void ANivel3Builder::SpawnVigias(AEnemyFactory* Factory, int32 Cantidad)
{
    for (int32 i = 0; i < Cantidad; i++)
    {
        FVector Pos = FVector(
...
void ANivel3Builder::SpawnHeraldos(AEnemyFactory* Factory, int32 Cantidad)
{
    for (int32 i = 0; i < Cantidad; i++)
    {
        FVector Pos = FVector(
...
void ANivel3Builder::SpawnBoyaElectrostatica(AEnemyFactory* Factory, int32 Cantidad)
{
    for (int32 i = 0; i < Cantidad; i++)
    {
        FVector Pos = FVector(
...
void ANivel3Builder::SpawnNaves(AGeneradorAmbientacion* Gen, int32 Cantidad)
{
    for (int32 i = 0; i < Cantidad; i++)
    {
        FVector Pos = FVector(
...
void ANivel3Builder::SpawnSatelites(AGeneradorAmbientacion* Gen, int32 Cantidad)
{
    for (int32 i = 0; i < Cantidad; i++)
    {
        FVector Pos = FVector(
...
void ANivel3Builder::SpawnRestos(AGeneradorAmbientacion* Gen, int32 Cantidad)
{
    for (int32 i = 0; i < Cantidad; i++)
    {
        FVector Pos = FVector(
...
void ANivel3Builder::SpawnAsteroides(AGeneradorAmbientacion* Gen, int32 Cantidad)
{
    for (int32 i = 0; i < Cantidad; i++)
    {
        FVector Pos = FVector(
...
void ANivel3Builder::Tick(float DeltaTime)
{
...
void ANivel3Builder::VerificarYSpawnearBoss(UWorld* World)
{
    int32 Vivos = 0;
    ...
    UGameplayStatics::GetAllActorsOfClass(World, AAsteroide::StaticClass(), Encontrados);
    for (AActor* A : Encontrados) A->Destroy();
    ...
    if (GEngine)
        GEngine->AddOnScreenDebugMessage(-1, 3.f, FColor::Cyan,
            TEXT("[Nivel3] Ambientación eliminada."));

    // 2. Arena de combate
    World->SpawnActor<AArenaCombate>(
        AArenaCombate::StaticClass(), FVector::ZeroVector, FRotator::ZeroRotator, Params);

    // 3. Boss — TribunalTormenta
    ATribunalTormenta* Boss = World->SpawnActor<ATribunalTormenta>(
        ATribunalTormenta::StaticClass(), FVector::ZeroVector, FRotator::ZeroRotator, Params);
    ...
    if (Boss)
    {
        ...
        if (GEngine)
            GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red,
                TEXT("[Nivel3] OLA 1 COMPLETADA — ¡Arena y TribunalTormenta aparecen!"));
    }
}